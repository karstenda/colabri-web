...
import { ColabDocReceiveInitMessage, ColabDocReceiveMessage, ColabDocSendUpdateMessage } from "../../data/ColabDocMessage";
...
export function ColabDocProvider({ docId, children } : ColabDocProviderProps) {
  ...
  useEffect(() => {
    ...
    console.log("Requesting document load for docId:", docId, "userId:", userId);
  ...
  useEffect(() => {
    ...
    switch (msg.type) {
      ...
      case "init":
        ...
        const loroDocData = Uint8Array.from(atob(loroDocBase64), c => c.charCodeAt(0));
  ...
  return (
    <ColabDocContext.Provider value={{ docId, colabDoc: document, updateColabDoc }}>
      {children}
