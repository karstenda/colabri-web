...
import ManagePermissionModal, { ManagePermissionModalPayload } from "../../ManagePermissionModal/ManagePermissionModal";
...
const DocEditorSheetBlock: React.FC<DocEditorSheetBlockProps> = (props) => {
    ...
    const handleManageBlock = async () => {
        ...
        const blockAcls = controller.getBlockAclMap(docEditorBlockProps.loroContainerId);
        ...
        if (newAclMaps) {
            ...
            controller.patchBlockAclMap(docEditorBlockProps.loroContainerId, newAclMaps);
    ...
    const handleShift = (direction: 'up' | 'down') => {
        ...
        // Shift the block
        const success = controller.shiftBlock(docEditorBlockProps.loroContainerId, direction);
        if (success) {
            controller.commit();
        }
    }
    ...
    return <DocEditorBlock 
        {...docEditorBlockProps}
        showUpDownControls={true}
        onManageBlock={docEditorBlockProps.onManageBlock ? docEditorBlockProps.onManageBlock : handleManageBlock}
