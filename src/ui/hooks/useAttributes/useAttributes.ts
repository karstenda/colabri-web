...
export const attributeKeys = {
  ...
  detail: (orgId: string, attributeId: string) =>
    [...attributeKeys.details(), orgId, attributeId] as const,
  documentValues: (orgId: string, docId: string) =>
    [...attributeKeys.all, 'document-values', orgId, docId] as const,
};

// Stable empty array reference to avoid unnecessary re-renders
const EMPTY_ATTRIBUTES: never[] = [];
...
export const useAttributes = (orgId: string, enabled = true) => {
  ...
  return {
    attributes: data?.data ?? EMPTY_ATTRIBUTES,
    isLoading,
    error,
    refetch,
  };
...
export const useCreateAttribute = (orgId: string) => {
  ...
  const mutation = useMutation({
    mutationFn: (data: CreateAttributeRequest) =>
      apiClient.orgId.postAttribute(orgId, data),
    onSuccess: () => {
...
export const useDeleteAttribute = (orgId: string) => {
  ...
  const mutation = useMutation({
    mutationFn: (attributeId: string) =>
      apiClient.orgId.deleteAttribute(orgId, attributeId),
    onSuccess: () => {
 ...
 */
export const useDocumentAttributeValues = (
  orgId: string,
  docId: string,
  enabled = true,
) => {
 ...
 */
export const useCreateDocumentAttributeValues = (
  orgId: string,
  docId: string,
) => {
  const queryClient = useQueryClient();
 ...
 */
export const useUpdateDocumentAttributeValues = (
  orgId: string,
  docId: string,
) => {
  const queryClient = useQueryClient();
 ...
 */
export const useDeleteDocumentAttributeValues = (
  orgId: string,
  docId: string,
) => {
  const queryClient = useQueryClient();
