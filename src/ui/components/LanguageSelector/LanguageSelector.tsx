...
import { usePlatformContentLanguages, useContentLanguages } from '../../hooks/useContentLanguages/useContentLanguage';
import type { OrgContentLanguage, PlatformContentLanguage } from '../../../api/ColabriAPI';
...
interface LanguageSelectorProps {
   ...
   */
  onChange?: (value: string | string[] | LanguageOption | LanguageOption[] | null) => void;
...
export const LanguageSelector: React.FC<LanguageSelectorProps> = ({
  ...
  const { languages: platformLanguages, isLoading: isPlatformLoading } = usePlatformContentLanguages(
    scope === 'platform'
  );
  
  const { languages: orgLanguages, isLoading: isOrgLoading } = useContentLanguages(
    orgId || '',
    scope === 'organization' && !!orgId
  );
  
  // Determine which languages to use
  const languages = scope === 'platform' ? platformLanguages : orgLanguages;
  ...
  const normalizeValue = (val: string | LanguageOption): LanguageOption | null => {
  ...
  // Handle change
  const handleChange = (_event: React.SyntheticEvent, newValue: LanguageOption | LanguageOption[] | null) => {
    if (!onChange) return;
  ...
  return (
    <Autocomplete
      ...
      noOptionsText={
        isLoading  ? 'Loading...'  : 'No languages found'
      }
    />
  );
};
