...
export default function AssigneeSelector({
  ...
  } = useUsers(orgId, { limit: userLimit }, !groupsOnly && !customUsers);
  
  const { 
    groups: fetchedGroups = [], 
    isLoading: groupsLoading 
  } = useGroups(orgId, { limit: groupLimit }, !usersOnly && !customGroups);
  ...
  const loading = (usersLoading && !customUsers) || (groupsLoading && !customGroups);
  ...
  return (
    <Autocomplete
      ...
      groupBy={(!usersOnly && !groupsOnly) ? (option) => option.type === 'user' ? 'Users' : 'Groups' : undefined}
      sx={{
        width: '100%',
