...
import { FullStatementDocument } from "../../../api/ColabriAPI";
...
export type StatementsGridProps = {
    editable?: boolean;
    handleClick?: (params: FullStatementDocument) => void;
}
...
function StatementsGrid(props: StatementsGridProps) {
    ...
    const handleStatementDelete = React.useCallback(
        (statement: FullStatementDocument) => async () => {
            ...
            if (confirmed) {
                try {
                    ...
                    notifications.show(`Statement ${statementName} deleted successfully.`, {
                        severity: 'success',
                        autoHideDuration: 3000,
                    });
                } catch (deleteError) {
                    notifications.show(
                        `Failed to delete statement. Reason: ${(deleteError as Error).message}`,
                        {
                            severity: 'error',
                            autoHideDuration: 3000,
                        },
    ...
    const defaultHandleClick = React.useCallback((statement: FullStatementDocument) => {
