...
import { useAttributes, useDeleteAttribute } from '../../hooks/useAttributes/useAttributes';
...
export default function AttributeListPage() {
  ...
  const { deleteAttribute, isPending: isDeletePending } = useDeleteAttribute(organization?.id || '');
  ...
  const handleRowClick = React.useCallback<GridEventListener<'rowClick'>>(
    ({ row }) => {
      navigate(`/org/${organization?.id}/attributes/${row.id}`);
    },
    [navigate, organization],
  );

  const handleCreateClick = React.useCallback(() => {
    navigate(`/org/${organization?.id}/attributes/new`);
  }, [navigate, organization]);

  const handleRowEdit = React.useCallback(
    (attribute: Attribute) => () => {
      navigate(`/org/${organization?.id}/attributes/${attribute.id}/edit`);
    },
    [navigate, organization],
  );
